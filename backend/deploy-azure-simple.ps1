...
# Function to run Azure CLI commands with better error handling
function Run-AzureCommand {
    param([string]$Command)
    
    Write-Host "Running: $Command"
    $result = & $azPath $Command.Split(' ') 2>&1
...
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings ASPNETCORE_ENVIRONMENT=Production"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings ConnectionStrings__DefaultConnection=`"$connectionString`""
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Jwt__SecretKey=241RunnersAwareness-Super-Secret-Key-2025-Production-Ready-Change-In-Production"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Jwt__Issuer=241RunnersAwareness"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Jwt__Audience=241RunnersAwareness"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Jwt__ExpiryInDays=7"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Jwt__RefreshTokenExpiryInDays=30"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__BaseUrl=https://241runnersawareness.org"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__ApiUrl=https://$AppServiceName.azurewebsites.net"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__Environment=Production"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__Version=1.0.0"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__MaxFileUploadSize=10485760"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings App__AllowedFileTypes=.jpg,.jpeg,.png,.pdf,.doc,.docx"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings Cors__AllowedOrigins=https://241runnersawareness.org,https://www.241runnersawareness.org,https://app.241runnersawareness.org"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings RateLimiting__PermitLimit=100"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings RateLimiting__Window=00:01:00"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings RateLimiting__SegmentsPerWindow=10"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings HealthChecks__DatabaseTimeout=00:00:05"
Run-AzureCommand "webapp config appsettings set --name $AppServiceName --resource-group $ResourceGroupName --settings HealthChecks__MemoryThreshold=1024"
