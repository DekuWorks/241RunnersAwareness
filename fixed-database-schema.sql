-- Fixed Database Schema Script - No References to AdditionalRoles Before Creation
...
-- 3. Update existing admin users to have proper AdditionalRoles (now that column exists)
UPDATE Users 
...
-- 5. Verify all admin users are properly configured (now that AdditionalRoles exists)
PRINT ''
...
-- 6. Check total user count (using only existing columns)
PRINT ''
