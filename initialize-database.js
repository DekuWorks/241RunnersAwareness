...
// Initialize database
async function initializeDatabase(token) {
    return new Promise((resolve, reject) => {
        const options = {
            hostname: '241runners-api-v2.azurewebsites.net',
            port: 443,
            path: '/debug/initialize-database',
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
// Test endpoints after initialization
...
async function main() {
    ...
    try {
        ...
        console.log('📋 Step 2: Initializing database with all required tables...');
        const initResult = await initializeDatabase(token);
        console.log('✅ Database initialization:', initResult.message, '\n');

        // Step 3: Test endpoints
        console.log('📋 Step 3: Testing endpoints after initialization...');
        ...
        console.log('\n🎯 NEXT STEPS:');
        if (successRate >= 80) {
            console.log('✅ Database initialization successful! Most endpoints are working.');
            console.log('🔧 Next: Implement missing CRUD operations and file upload.');
        } else {
            console.log('⚠️  Some endpoints still failing. May need to check controller implementations.');
            console.log('🔧 Next: Review and fix controller logic for failing endpoints.');
        }
